...
func (conn *ClientConn) Connect(cfg *server.ServerConfig, sconn *server.ServerConn) error {

	if err := conn.handshake(cfg, sconn); err != nil {
		log.Errorf("ClientConn.Connect error: %v", err)
		conn.Close()
		return err
	}
...
func (c *ClientConn) handshake(cfg *server.ServerConfig, sconn *server.ServerConn) error {
	...
	if err = binary.Read(c.conn, binary.BigEndian, &numSecurityTypes); err != nil {
		...
		serverSecurityfailure(cfg, sconn, err)
		log.Errorf("Error reading security types: %v", err)
		return err
	}
	...
	//server端的认证
	if err := serverSecurityHandler(cfg, sconn); err != nil {
		//sconn.Close()
		return err
	}
...
func (c *ClientConn) mainLoop() {
	...
	defer func() {
		log.Warningln("ClientConn.MainLoop: exiting!")
	...
	for {
		...
		if err := binary.Read(c.conn, binary.BigEndian, &messageType); err != nil {
			log.Errorf("ClientConn.MainLoop: error reading messagetype, %s", err)
			break
		}

		msg, ok := typeMap[messageType]
		if !ok {
			log.Errorf("ClientConn.MainLoop: bad message type, %d", messageType)
			// Unsupported message type! Bad!
			break
		}
		log.Debugf("ClientConn.MainLoop: got ServerMessage:%s", common.ServerMessageType(messageType))
		...
		if err != nil {
			log.Errorf("ClientConn.MainLoop: error parsing message, %s", err)
			break
		}
		log.Debugf("ClientConn.MainLoop: read & parsed ServerMessage:%d, %s", parsedMsg.Type(), parsedMsg)
	}
}
...
func serverSecurityfailure(cfg *server.ServerConfig, c *server.ServerConn, clientErr error) error {
	if err := binary.Write(c, binary.BigEndian, uint8(0)); err != nil {
		log.Errorf("serverSecurityfailure write num of security types err %v", err)
		return err
	}

	if err := binary.Write(c, binary.BigEndian, uint32(len(clientErr.Error()))); err != nil {
		return err
	}
	if err := binary.Write(c, binary.BigEndian, []byte(clientErr.Error())); err != nil {
		return err
	}
	log.Infoln("emit server securityfailure event.")

	return clientErr
}

func serverSecurityHandler(cfg *server.ServerConfig, c *server.ServerConn) error {
	...
	var secType server.SecurityType
	if err := binary.Read(c, binary.BigEndian, &secType); err != nil {
		return err
	}

	secTypes := make(map[server.SecurityType]server.SecurityHandler)
