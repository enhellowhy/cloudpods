...
func ConnectFbsFile(filename string, conn *server.ServerConn) (*FbsReader, error) {
	...
	if err != nil {
		log.Errorf("failed to open fbs reader:", err)
		return nil, err
	}
	//NewFbsReader("/Users/amitbet/vncRec/recording.rbs")
	initMsg, err := fbs.ReadStartSession()
	if err != nil {
		log.Errorf("failed to open read fbs start session:", err)
		return nil, err
	}
...
func (h *FBSPlayListener) sendFbsMessage() {
	...
	if err != nil {
		log.Errorf("TestServer.NewConnHandler: Error in reading FBS segment: ", err)
		return
	}
	//common.IClientConn{}
	...
	if msg == nil {
		log.Errorf("TestServer.NewConnHandler: Error unknown message type: ", messageType)
		return
	}
