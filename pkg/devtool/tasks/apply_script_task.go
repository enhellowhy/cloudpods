...
func (self *ApplyScriptTask) OnInit(ctx context.Context, obj db.IStandaloneModel, body jsonutils.JSONObject) {
	...
	serverJson, err := modules.Servers.GetById(session, sa.GuestId, nil)
	if err != nil {
	...
	if serverDetail.CreatedAt.Add(30*time.Minute).After(time.Now()) || serverDetail.Status != compute.VM_RUNNING {
