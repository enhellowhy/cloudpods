...
func StoreBytes(bytes *bytes.Buffer, data []byte) {
	...
	if err != nil {
		log.Errorf("Error in encoding while saving bytes: ", err)
	}
}
...
func (t *TightEncoding) Read(pixelFmt *common.PixelFormat, rect *common.Rectangle, r *common.RfbReadHelper) (common.IEncoding, error) {
	...
	switch compType {
	case TightFill:
		log.Debugf("reading fill size=%d\n", bytesPixel)
		//read color
	...
	default:
		...
		if compType > TightJpeg {
			log.Debugln("Compression control byte is incorrect!")
		}
...
func handleTightFilters(subencoding uint8, pixelFmt *common.PixelFormat, rect *common.Rectangle, r *common.RfbReadHelper) {
	...
	if (subencoding & FILTER_ID_MASK) > 0 { // filter byte presence
		...
		log.Debugf("handleTightFilters: read filter: %d\n", filterid)
	}

	bytesPixel := calcTightBytePerPixel(pixelFmt)

	log.Debugf("handleTightFilters: filter: %d\n", filterid)
	...
	case TightFilterPalette: //PALETTE_FILTER
		...
		log.Debugf("handleTightFilters: ----PALETTE_FILTER: paletteSize=%d bytesPixel=%d\n", paletteSize, bytesPixel)
		//complete palette
		...
		if err != nil {
			log.Errorf("handleTightFilters: error in handling tight encoding, Reading Palette: %v", err)
			return
		}

	case TightFilterGradient: //GRADIENT_FILTER
		log.Debugf("----GRADIENT_FILTER: bytesPixel=%d\n", bytesPixel)
	...
	case TightFilterCopy: //BASIC_FILTER
		...
		log.Debugf("handleTightFilters: ----BASIC_FILTER: bytesPixel=%d", bytesPixel)
