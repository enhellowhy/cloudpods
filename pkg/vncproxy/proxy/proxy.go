...
import (
	...
	listeners "yunion.io/x/onecloud/pkg/vncproxy/recorder"
)
...
func (vp *VncProxy) getProxySession(sessionId string) (*VncSession, error) {
	...
	if !vp.UsingSessions {
		if vp.SingleSession == nil {
			log.Errorf("SingleSession is empty, use sessions or populate the SingleSession member of the VncProxy struct.")
		}
...
func (vp *VncProxy) newServerConnHandler(cfg *server.ServerConfig, sconn *server.ServerConn) error {
	...
	if err != nil {
		log.Errorf("Proxy.newServerConnHandler can't get session: %d", sconn.SessionId)
		return err
	}

	var rec *listeners.Recorder

	timeCST := time.Now().In(time.FixedZone("CST", 8*3600))
	datePath := timeCST.Format(timeutils.DateFormat)
	instancePath := filepath.Join(options.Options.S3MountPoint, datePath, vp.SingleSession.InstanceName)
	if !fileutils.Exists(instancePath) {
		err := os.MkdirAll(instancePath, 0755)
		if err != nil {
			log.Warningf("fail to create instance dir %s: %s", instancePath, err)
			return err
		}
	}
	...
	if session.Type == SessionTypeRecordingProxy {
		recFile := vp.SingleSession.Cred.GetUserName() + "_" + timeCST.Format(timeutils.NormalTimeFormat) + "_" + vp.SingleSession.ID + ".rbs"
		recPath = path.Join(instancePath, recFile)
		rec, err = listeners.NewRecorder(recPath)
		if err != nil {
			log.Errorf("Proxy.newServerConnHandler can't open recorder save path: %s", recPath)
			return err
		}
	...
	if session.Type == SessionTypeProxyPass || session.Type == SessionTypeRecordingProxy {
		...
		if err != nil {
			...
			log.Errorf("Proxy.newServerConnHandler error creating connection: %s", err)
			return err
		}
		if session.Type == SessionTypeRecordingProxy {
		...
		err = cconn.Connect(cfg, sconn)
		if err != nil {
			rec.Close()
			os.Remove(recPath)
			session.Status = SessionStatusError
			log.Errorf("Proxy.newServerConnHandler error connecting to client: %s", err)
			return err
		}
...
func (vp *VncProxy) StartListening() {
	...
	if vp.TCPListeningURL != "" && vp.WsListeningURL != "" {
		log.Infof("running two listeners: tcp port: %s, ws url: %s", vp.TCPListeningURL, vp.WsListeningURL)
	...
	if vp.WsListeningURL != "" {
		log.Infof("running ws listener url: %s", vp.WsListeningURL)
		server.WsServe(vp.WsListeningURL, cfg)
	}
	if vp.TCPListeningURL != "" {
		log.Infof("running tcp listener on port: %s", vp.TCPListeningURL)
		server.TcpServe(vp.TCPListeningURL, cfg)
	}
}

func (vp *VncProxy) WebsocketHandler() websocket.Handler {
	...
	cfg := &server.ServerConfig{
		...
		Width:            uint16(1024),
		NewConnHandler:   vp.newServerConnHandler,
		UseDummySession:  false,
		SessionId:        vp.SingleSession.ID,
	}

	log.Infof("running ws handler for %s(%s:%s)", vp.SingleSession.InstanceName, vp.SingleSession.TargetHostname, vp.SingleSession.TargetPort)
	return server.WsHandle(cfg)
}
