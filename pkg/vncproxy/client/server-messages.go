...
func (fbm *MsgFramebufferUpdate) Read(c common.IClientConn, r *common.RfbReadHelper) (common.ServerMessage, error) {
	...
	log.Debugf("MsgFramebufferUpdate.Read: numrects= %d", numRects)

	rects := make([]common.Rectangle, numRects)
	for i := uint16(0); i < numRects; i++ {
		log.Debugf("MsgFramebufferUpdate.Read: ###############rect################: %d", i)
		...
		log.Debugf("MsgFramebufferUpdate.Read: rect# %d, rect hdr data: enctype=%s, data: %s", i, encType, string(jBytes))
		...
		if supported {
			...
			if strings.Contains(encType.String(), "Pseudo") {
				...
				log.Errorf("MsgFramebufferUpdate.Read: unsupported encoding type: %d, %s", encodingTypeInt, encType)
