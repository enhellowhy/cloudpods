...
func NewRecorder(saveFilePath string) (*Recorder, error) {
	...
	if err != nil {
		log.Errorf("unable to open file: %s, error: %v", saveFilePath, err)
		return nil, err
	}
	...
	go func() {
		for {
			data, ok := <-rec.segmentChan
			if !ok {
				log.Infof("recorder %s segmentChan has been closed, close count is %d", rec.RBSFileName, rec.closeCount)
				return
			}
...
func (r *Recorder) writeStartSession(initMsg *common.ServerInit) error {
	...
	//log.Debugf(">>>>>>buffer for initMessage:%v ", buff.Bytes())
...
func (r *Recorder) Consume(data *common.RfbSegment) error {
	...
	case r.segmentChan <- data:
		// default:
		// 	log.Error("error: recorder queue is full")
	}
...
func (r *Recorder) HandleRfbSegment(data *common.RfbSegment) error {
	defer func() {
		if r := recover(); r != nil {
			log.Error("Recovered in HandleRfbSegment: ", r)
		}
	}()
	...
	case common.SegmentMessageStart:
		if !r.sessionStartWritten {
			log.Debugf("Recorder.HandleRfbSegment: writing start session segment: %v", r.serverInitMessage)
		...
		case common.FramebufferUpdate:
			log.Debugf("Recorder.HandleRfbSegment: saving FramebufferUpdate segment")
			//r.writeToDisk()
		case common.SetColourMapEntries:
		case common.Bell:
		case common.ServerCutText:
		default:
			log.Warningf("Recorder.HandleRfbSegment: unknown message type:" + string(data.UpcomingObjectType))
		}
	...
	case common.SegmentRectSeparator:
		log.Debugf("Recorder.HandleRfbSegment: writing rect")
		//r.writeToDisk()
	case common.SegmentBytes:
		log.Debug("Recorder.HandleRfbSegment: writing bytes, len:", len(data.Bytes))
	...
	case common.SegmentFullyParsedClientMessage:
		...
		case common.SetPixelFormatMsgType:
			...
			log.Debugf("Recorder.HandleRfbSegment: client message %v", *clientMsg)
...
func (r *Recorder) writeToDisk() error {
	...
	//log.Debugf("paddedSize=%d paddingSize=%d bytesLen=%d", paddedSize, paddingSize, bytesLen)
	//write buffer padded to 32bit
	_, err := r.buffer.WriteTo(r.writer)
	padding := make([]byte, paddingSize)
	//log.Debugf("padding=%v ", padding)
