	...
	for {
		...
		default:
			...
			if err := binary.Read(c, binary.BigEndian, &messageType); err != nil {
				log.Errorf("ServerConn.handle error: %v", err)
				return err
			}
			log.Debugf("ServerConn.handle: got messagetype, %d", messageType)
			msg, ok := clientMessages[messageType]
			log.Debugf("ServerConn.handle: found message type, %v", ok)
			if !ok {
				log.Errorf("ServerConn.handle: unsupported message-type: %v", messageType)
			}
			parsedMsg, err := msg.Read(c)
			log.Debugf("ServerConn.handle: got parsed messagetype, %v", parsedMsg)
			...
			if err != nil {
				log.Errorf("srv err %s", err.Error())
				return err
			}

			log.Debugf("IServerConn.Handle got ClientMessage: %s, %v", parsedMsg.Type(), parsedMsg)
			...
			if err != nil {
				log.Errorf("IServerConn.Handle: listener consume err %s", err.Error())
				return err
			}
		}
	}
