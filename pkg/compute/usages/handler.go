...
type objUsageFunc func(rbacutils.TRbacScope, mcclient.IIdentityProvider, bool, []db.IStandaloneModel, []string, []string, []string, string, bool, string, string) (Usage, error)
...
func rangeObjHandler(
	...
	return func(ctx context.Context, w http.ResponseWriter, r *http.Request) {
		...
		key := getCacheKey(scope, ownerId, isOwner, rangeObjs, hostTypes, providers, brands, cloudEnv, includeSystem, from, interval)
		if !refresh {
		...
		usage, err := reporter(scope, ownerId, isOwner, rangeObjs, hostTypes, providers, brands, cloudEnv, includeSystem, from, interval)
		if err != nil {
			httperrors.GeneralServerError(ctx, w, err)
			return
		}
...
func ReportHostUsage(scope rbacutils.TRbacScope, userCred mcclient.IIdentityProvider, isOwner bool, hosts []db.IStandaloneModel, hostTypes []string, providers []string, brands []string, cloudEnv string, includeSystem bool, from, interval string) (Usage, error) {
	return ReportGeneralUsage(scope, userCred, isOwner, hosts, hostTypes, providers, brands, cloudEnv, includeSystem, from, interval)
}

func ReportWireUsage(scope rbacutils.TRbacScope, userCred mcclient.IIdentityProvider, isOwner bool, wires []db.IStandaloneModel, hostTypes []string, providers []string, brands []string, cloudEnv string, includeSystem bool, from, interval string) (Usage, error) {
	return ReportGeneralUsage(scope, userCred, isOwner, wires, hostTypes, providers, brands, cloudEnv, includeSystem, from, interval)
}

func ReportCloudAccountUsage(scope rbacutils.TRbacScope, userCred mcclient.IIdentityProvider, isOwner bool, accounts []db.IStandaloneModel, hostTypes []string, providers []string, brands []string, cloudEnv string, includeSystem bool, from, interval string) (Usage, error) {
	return ReportGeneralUsage(scope, userCred, isOwner, accounts, hostTypes, providers, brands, cloudEnv, includeSystem, from, interval)
}

func ReportCloudProviderUsage(scope rbacutils.TRbacScope, userCred mcclient.IIdentityProvider, isOwner bool, managers []db.IStandaloneModel, hostTypes []string, providers []string, brands []string, cloudEnv string, includeSystem bool, from, interval string) (Usage, error) {
	return ReportGeneralUsage(scope, userCred, isOwner, managers, hostTypes, providers, brands, cloudEnv, includeSystem, from, interval)
}

func ReportSchedtagUsage(scope rbacutils.TRbacScope, userCred mcclient.IIdentityProvider, isOwner bool, schedtags []db.IStandaloneModel, hostTypes []string, providers []string, brands []string, cloudEnv string, includeSystem bool, from, interval string) (Usage, error) {
	return ReportGeneralUsage(scope, userCred, isOwner, schedtags, hostTypes, providers, brands, cloudEnv, includeSystem, from, interval)
}

func ReportClusterUsage(scope rbacutils.TRbacScope, userCred mcclient.IIdentityProvider, isOwner bool, clusters []db.IStandaloneModel, hostTypes []string, providers []string, brands []string, cloudEnv string, includeSystem bool, from, interval string) (Usage, error) {
	return ReportGeneralUsage(scope, userCred, isOwner, clusters, hostTypes, providers, brands, cloudEnv, includeSystem, from, interval)
}

func ReportZoneUsage(scope rbacutils.TRbacScope, userCred mcclient.IIdentityProvider, isOwner bool, zones []db.IStandaloneModel, hostTypes []string, providers []string, brands []string, cloudEnv string, includeSystem bool, from, interval string) (Usage, error) {
	return ReportGeneralUsage(scope, userCred, isOwner, zones, hostTypes, providers, brands, cloudEnv, includeSystem, from, interval)
}

func ReportCloudRegionUsage(scope rbacutils.TRbacScope, userCred mcclient.IIdentityProvider, isOwner bool, cloudRegions []db.IStandaloneModel, hostTypes []string, providers []string, brands []string, cloudEnv string, includeSystem bool, from, interval string) (Usage, error) {
	return ReportGeneralUsage(scope, userCred, isOwner, cloudRegions, hostTypes, providers, brands, cloudEnv, includeSystem, from, interval)
}

//func ReportCloudProviderUsersUsage(from, interval string, managers []db.IStandaloneModel) (Usage, error) {
//	return getXskyUsersUsage(from, interval, managers)
//}
//
	...
	if scope == rbacutils.ScopeSystem {
		if len(rangeObjs) > 0 && rangeObjs[0].Keyword() == "cloudaccount" {
			...
			if cloudAccount.Brand == api.CLOUD_PROVIDER_XSKY {
				count, err = getSystemCloudAccountXskyUsage(userCred, rangeObjs, hostTypes, providers, brands, cloudEnv, includeSystem)
				return
			}
		}
		if len(rangeObjs) > 0 && rangeObjs[0].Keyword() == "cloudprovider" {
			...
			if cloudProvider.Provider == api.CLOUD_PROVIDER_XSKY {
				count, err = getSystemCloudProviderXskyUsage(userCred, rangeObjs, hostTypes, providers, brands, cloudEnv, includeSystem)
				return
			}
		}
