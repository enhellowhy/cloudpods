...
func wsHandlerFunc(ws io.ReadWriter, cfg *ServerConfig, sessionId string) {
	...
	if err != nil {
		...
		log.Errorf("Error attaching new connection. %v", err)
	}
}

func WsHandle(cfg *ServerConfig) websocket.Handler {
	server := WsServer{cfg}
	return server.Handler(WsHandler(wsHandlerFunc))
}
...
func attachNewServerConn(c io.ReadWriter, cfg *ServerConfig, sessionId string) error {
	...
	if err := ServerVersionHandler(cfg, conn); err != nil {
		log.Errorf("server handle version err: %v", err)
		conn.Close()
		return err
	}
	//
