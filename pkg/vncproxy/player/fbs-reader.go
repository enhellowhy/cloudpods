...
func (fbs *FbsReader) Read(p []byte) (n int, err error) {
	if fbs.buffer.Len() < len(p) {
		...
		if err != nil {
			log.Errorf("FBSReader.Read: error reading FBSsegment: ", err)
			return 0, err
		}
...
func NewFbsReader(fbsFile string) (*FbsReader, error) {
	...
	if err != nil {
		log.Errorf("NewFbsReader: can't open fbs file: ", fbsFile)
		return nil, err
	}
...
func (fbs *FbsReader) ReadStartSession() (*common.ServerInit, error) {
	...
	if err != nil {
		log.Errorf("FbsReader.ReadStartSession: error reading rbs init message - FBS file Version:", err)
		return nil, err
	}
	...
	if err != nil {
		log.Errorf("FbsReader.ReadStartSession: error reading rbs init - RFB Version: ", err)
		return nil, err
	}
	...
	if err != nil {
		log.Errorf("FbsReader.ReadStartSession: error reading rbs init - SecType: ", err)
	}

	//read frame buffer width, height
	binary.Read(fbs, binary.BigEndian, &framebufferWidth)
	if err != nil {
		log.Errorf("FbsReader.ReadStartSession: error reading rbs init - FBWidth: ", err)
		return nil, err
	}
	...
	if err != nil {
		log.Errorf("FbsReader.ReadStartSession: error reading rbs init - FBHeight: ", err)
		return nil, err
	}
	...
	if err != nil {
		log.Errorf("FbsReader.ReadStartSession: error reading rbs init - Pixelformat: ", err)
		return nil, err
	}
	...
	if err != nil {
		log.Errorf("FbsReader.ReadStartSession: error reading rbs init - deskname Len: ", err)
		return nil, err
	}
	...
	fbs.Read(bytes)
	if err != nil {
		log.Errorf("FbsReader.ReadStartSession: error reading rbs init - desktopName: ", err)
		return nil, err
	}
...
func (fbs *FbsReader) ReadSegment() (*FbsSegment, error) {
	...
	if err != nil {
		log.Errorf("FbsReader.ReadStartSession: read len, error reading rbs file: ", err)
		return nil, err
	}
	...
	_, err = reader.Read(bytes)
	if err != nil {
		log.Errorf("FbsReader.ReadSegment: read bytes, error reading rbs file: ", err)
		return nil, err
	}
	...
	if err != nil {
		log.Errorf("FbsReader.ReadSegment: read timestamp, error reading rbs file: ", err)
		return nil, err
	}
